---
name: nhdesign3-homepage-proposal
description: Generate an NH desktop-website feature proposal deliverable — a PPT 기획서 (slide deck) plus a 화면 흐름 개요 (screen flow diagram) — in the current live brand style of 나무증권/mynamuh.com and NH투자증권/nhsec.com (currently the same site), built from live-site-derived layout/color/nav facts recorded in nhdesign3 (v3 series — reads nhdesign3_* tables; not to be confused with a future v4 sibling). Trigger: "/nhdesign3-homepage-proposal <feature/area>", or any request to make a "기획서", "화면흐름개요", "웹버전 기획서", "웹 화면흐름도", "홈페이지 기획서" for a 나무증권/mynamuh/nhsec.com desktop website feature. For mobile app or mobile-web (NDS) requests, use `nhdesign3-nds-proposal` instead.
---

# nhdesign3-homepage-proposal

Website-brand sibling of `nhdesign3-nds-proposal`. Produces the **same two-part deliverable shape** — a
...
This skill is a thin orchestrator, same as `nhdesign3-nds-proposal`. It hands off to `nhdesign3` for DB
...
3. Pull the slide/flow template pattern and every website source row in one query:
   ...
   pattern (`screen-flow-overview-pattern`) — both shared verbatim with `nhdesign3-nds-proposal`, reused
...
Same two cases as `nhdesign3-nds-proposal`, adapted:
...
Same shape as `nhdesign3-nds-proposal` step 2 — left-to-right compact-scale screen row, numbered-circle
...
Building this before the slides surfaces missing states the same way it does for `nhdesign3-nds-proposal` —
...
Same three-part shape as `nhdesign3-nds-proposal` step 3 — Cover / one content slide per primary-flow
...
## 5. What must differ from `nhdesign3-nds-proposal`'s BUILD step (the substance swap)
...
- **No component-instance lookup step.** `nhdesign3-nds-proposal` has a "BUILD-mode component lookup"
...
different from `nhdesign3-nds-proposal`'s Figma-to-Figma screenshot compare:
...
- Separately, screenshot-compare the finished flow overview and slide deck against the
  ...
  화면경로/화면구분 box structure, arrow style) — this part is identical to `nhdesign3-nds-proposal`'s
...
`nhdesign3-nds-proposal` uses for BUILD lessons. If the brand/redirect state has flipped again since the
...
**Dedicated entry point**: `~/.claude/skills/nhdesign3-knowhow-homepage/SKILL.md` wraps this
...
**Trigger**: any time a user correction during a `nhdesign3-homepage-proposal` BUILD session reveals
...
**How to write**:
1. Insert one page per lesson into the `knowhow-homepage-proposal` source (`kind='knowhow'`,
   same `portmanagement` Supabase project `nhdesign3` uses) — `page_name` states the lesson as a
...
3. Register/extend a `nhdesign3_topics` row so the lesson is keyword-findable, and bump
   `knowhow-homepage-proposal`'s `total_pages`/`learned_pages`.
...
This source is kept deliberately separate from `knowhow-nhdesign3` (the NDS/app-side knowhow
used by `nhdesign3-nds-proposal`, captured via `nhdesign3-knowhow-app`) — desktop-website lessons
...
- Not for mobile app or mobile-web (NDS) requests — redirect those to `nhdesign3-nds-proposal`. If a
