...
status: shipped ("반영완료⭐️" in file name — this is production-shipped, not a draft; highest-confidence reference)
---
...
## Screen inventory (from the master page, 42 top-level frames)
...
4. **Category tabs** (`tab_2depth_button`, horizontally scrollable, 7 `tab_btn` instances): **전체 · 유상 · 매수청구 · 배당 · 무상 · (기타 types)** — 2nd-depth filter by right-type. Selected tab is bold black + underline; others gray.
...
4. `[candidate-common]` Use single-button layer popups for informational/navigational heads-ups; reserve the two-button (취소/제출) pattern strictly for irreversible confirmations. Quote popup copy verbatim when it maps to a real flow (see exact strings above) rather than paraphrasing.
